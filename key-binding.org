...
* magit:
| s | magit-stage-item          | magit-status |   |
| u | magit-unstage-item        | magit-status |   |
| i | magit-ignore-item         | magit-status |   |
| I | magit-ignore-item-locally | magit-status |   |
| j | magit-section-jump-map    | magit-status |   |
| . | magit-mark-item           | magit-status |   |
| = | magit-diff-with-mark      | magit-status |   |
| k | magit-discard-item        | magit-status |   |
| C | magit-commit-add-log      | magit-status |   |
